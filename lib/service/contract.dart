...
/// A list of contract names that are expected to be deployed on the blockchain
/// token: a fungible token contract
/// identities: DID (Decentralized Identity) contract
/// nft-dcep: digital collectible or DCEP (Digital Currency Electronic Payment) NFT.
const List<String> contractsOnChain = ['token', 'identities', 'nft-dcep'];
...
class ContractService {
  ...
  /// Loads contract data asynchronously from an API
  static Future<ContractService> init() async {
    final Map<String, Contract> contracts = {};
    for (final name in contractsOnChain) {
      (await Contract.fromApi(name)).ifPresent((contract) => contracts[name] = contract);
    }
...
class Contract {
  ...
  Future<bool> sendTransaction(String privateKey, String functionName, List<dynamic> parameters) async {
    ...
    final String hash = await web3Client.credentialsFromPrivateKey(privateKey).then(
