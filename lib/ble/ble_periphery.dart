...
class BleCentralState extends EnumClass {
  ...
  const BleCentralState._(super.name);
}
...
class BlePeriphery {
  ...
  factory BlePeriphery() {
    ...
    const EventChannel stateChannel = EventChannel('matrix.ble_periphery/state');
    const MethodChannel methodChannel = MethodChannel('matrix.ble_periphery/method');
  ...
  Stream<Map<String, dynamic>> stateStream() {
    return _stateChannel.receiveBroadcastStream()
      .asyncMap(
        (state) => (state as Map<dynamic, dynamic>).map((key, value) => MapEntry(key as String, value)),
      );
  }

  Stream<Map<String, dynamic>> dataStream() {
    return _dataChannel.receiveBroadcastStream()
      .asyncMap(
        (data) => (data as Map<dynamic, dynamic>).map((key, value) => MapEntry(key as String, value)),
      );
  }
  ...
  Future<void> sendData(String device, Uint8List data) {
    return _methodChannel.invokeMethod('sendData', {'device': device, 'data': data});
  }
}
